...
Se il testo dell'esercizio dice "23,5 km", tu DEVI usare "23,5 km" — mai "24 km", "23 km", "circa 24 km" o qualsiasi altra approssimazione.`;
    ...
    if (isProceduralDivisionSession(taskContext, messages)) {
      contextPrompt += lang === 'en'
        ? `\n\nCOLUMN OPERATION VISUAL CONTRACT: Use [COLONNA: tipo=division, numeri=X,Y] for complete operations. For interactive guided mode use [COLONNA: tipo=division, numeri=X,Y, parziale=true, celle_compilate=N] where N = number of result cells filled. Highlight with evidenzia=cellId:color (green=student found it, orange=hint, blue=coach gave it). Follow the 3-PHASE FLOW: Phase 1=theory intro, Phase 2=simple example (show complete), Phase 3=real exercise (student-led with partial tag). NEVER show result before student finds it. ALWAYS ask "What would you do?" before guiding.`
        : `\n\nCONTRATTO VISIVO OPERAZIONI IN COLONNA: Usa [COLONNA: tipo=divisione, numeri=X,Y] per operazioni complete. Per la guida interattiva usa [COLONNA: tipo=divisione, numeri=X,Y, parziale=true, celle_compilate=N] dove N = cifre del risultato compilate. Evidenzia con evidenzia=cellId:colore (verde=trovato dallo studente, arancione=hint, blu=dato dal coach). Segui il FLUSSO A 3 FASI: Fase 1=intro teorica con esempio vita reale, Fase 2=esempio semplice (mostra completo), Fase 3=esercizio reale (guidato dallo studente con tag parziale). MAI mostrare il risultato prima che lo studente lo trovi. SEMPRE chiedere "Cosa faresti?" prima di guidare.`;
    }

    // Always inject COLONNA tag instructions
    contextPrompt += lang === 'en'
      ? `\n\nCOLUMN OPERATION FORMATTING (MANDATORY): For any column operation use [COLONNA: tipo=multiplication, numeri=754,27]. For partial/interactive: [COLONNA: tipo=division, numeri=765,2, parziale=true, celle_compilate=0]. For highlighting: add evidenzia=qp0:verde. Colors: green/verde, orange/arancione, blue/blu. NEVER use ASCII art, pipes or dashes.`
      : `\n\nFORMATTAZIONE OPERAZIONI IN COLONNA (OBBLIGATORIO): Per qualsiasi operazione in colonna usa [COLONNA: tipo=moltiplicazione, numeri=754,27]. Per modalità interattiva: [COLONNA: tipo=divisione, numeri=765,2, parziale=true, celle_compilate=0]. Per evidenziare: aggiungi evidenzia=qp0:verde. Colori: verde, arancione, blu. NON usare MAI ASCII art, pipe (|), trattini (---) o blocchi di codice.`;
    ...
    const model = hasSourceImage ? "google/gemini-2.5-pro" : hasImages ? "google/gemini-2.5-flash" : "google/gemini-2.5-flash";
