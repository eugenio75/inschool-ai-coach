        ...
        <button onClick={onClick} className="w-full text-left">
          <div className="flex items-start justify-between mb-3 pr-6">
            <div className="flex items-center gap-2">
              {isStudy && (
                <span className="flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-bold uppercase tracking-wider bg-accent text-accent-foreground">
                  <BookOpen className="w-3 h-3" /> Studio
                </span>
              )}
              {!isStudy && task.task_type === "exercise" && (
                <span className="flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-bold uppercase tracking-wider bg-clay-light text-clay-dark">
                  <PenLine className="w-3 h-3" /> Esercizio
                </span>
              )}
              <span className={`px-3 py-1 rounded-full text-[12px] font-bold uppercase tracking-wider ${colorClass}`}>
          ...
          <h3 className={`font-display text-xl font-medium mb-1.5 ${task.completed ? "line-through text-muted-foreground" : "text-foreground"}`}>
            {task.title}
          </h3>
          <p className="text-[14px] text-foreground/65 mb-3 leading-snug">{task.description}</p>

          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5 text-[13px] text-foreground/60">
              <Clock className="w-3.5 h-3.5" />
              {task.completed ? "Completato" : `${task.steps} passi da fare`}
            </div>
            <div className="flex items-center gap-2">
              {task.sessionStatus === "paused" && !task.completed && (
                <span className="flex items-center gap-1 text-[11px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400">
                  ⏸ In pausa
                </span>
              )}
              {task.sessionStatus === "active" && !task.completed && (
                <span className="flex items-center gap-1 text-[11px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-sky-100 text-sky-700 dark:bg-sky-900/30 dark:text-sky-400">
                  ▶ In corso
                </span>
              )}
              {task.completed ? (
                <div className="flex items-center gap-1 text-primary text-[13px] font-medium bg-primary/10 px-2.5 py-1 rounded-full">
                  <Check className="w-3.5 h-3.5" />
                  Fatto
                </div>
              ) : (
                <div className="flex items-center gap-1 text-primary text-[13px] font-semibold">
