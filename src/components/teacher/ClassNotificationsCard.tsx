      ...
      <div className="rounded-[20px] bg-card border border-border/60 overflow-hidden">
        {notifications.length === 0 ? (
          <div className="p-6 text-center">
            <p className="text-[13px] text-muted-foreground">Nessuna notifica al momento.</p>
          </div>
        ) : (
          <div className="divide-y divide-border/60">
            {notifications.map((n) => {
              ...
              return (
                ...
                >
                  <span className={cn("shrink-0 h-9 w-9 rounded-[10px] flex items-center justify-center", styles.iconBg)}>
                    <Icon className={cn("h-4 w-4", styles.iconText)} />
                  </span>
                  <div className="flex-1 min-w-0">
                    <p className="text-[14px] font-semibold text-foreground leading-tight truncate">{n.title}</p>
                    <p className="text-[12px] text-muted-foreground mt-0.5 leading-tight truncate">{n.subtitle}</p>
                  </div>
                  <span className={cn(
