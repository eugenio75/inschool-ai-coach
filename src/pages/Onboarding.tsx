...
const Onboarding = () => {
  ...
  const [data, setData] = useState<OnboardingData>({
    name: "", age: "", schoolLevel: "", favoriteSubjects: [], difficultSubjects: [],
    struggles: [], focusTime: "15", supportStyle: "",
  });
  ...
  const renderStep = () => {
    switch (step) {
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <input type="text" placeholder="Il tuo nome" value={data.name} onChange={(e) => setData({ ...data, name: e.target.value })} className="w-full px-4 py-3 rounded-2xl border border-border bg-card text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 text-lg" />
              <div>
      ...
      case 1:
        return (
          <div className="space-y-6">
            <div><h2 className="font-display text-2xl font-bold text-foreground mb-2">Che classe fai, {data.name}?</h2><p className="text-muted-foreground">Adatteremo tutto al tuo livello.</p></div>
            <div className="space-y-3">
              {schoolLevels.map((level) => (
                <button key={level.id} onClick={() => setData({ ...data, schoolLevel: level.id })} className={`w-full text-left px-5 py-4 rounded-2xl border transition-all ${data.schoolLevel === level.id ? "border-primary bg-sage-light text-foreground shadow-soft" : "border-border bg-card text-foreground hover:bg-muted"}`}>
      ...
      case 2:
        return (
          <div className="space-y-6">
            <div><h2 className="font-display text-2xl font-bold text-foreground mb-2">Quali materie ti piacciono?</h2><p className="text-muted-foreground">Seleziona quelle che preferisci.</p></div>
            <div>
              <label className="text-sm font-medium text-foreground mb-3 block">Ti piacciono 💚</label>
              <div className="flex flex-wrap gap-2 mb-6">
                {subjects.map((subj) => (
                  <button key={`fav-${subj}`} onClick={() => setData({ ...data, favoriteSubjects: toggleInArray(data.favoriteSubjects, subj) })} className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${data.favoriteSubjects.includes(subj) ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:bg-accent"}`}>{subj}</button>
                ))}
              </div>
              <label className="text-sm font-medium text-foreground mb-3 block">Sono difficili 🤔</label>
              <div className="flex flex-wrap gap-2">
                {subjects.map((subj) => (
                  <button key={`diff-${subj}`} onClick={() => setData({ ...data, difficultSubjects: toggleInArray(data.difficultSubjects, subj) })} className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${data.difficultSubjects.includes(subj) ? "bg-terracotta text-destructive-foreground" : "bg-muted text-muted-foreground hover:bg-accent"}`}>{subj}</button>
                ))}
              </div>
            </div>
          </div>
      ...
      case 3:
        return (
          <div className="space-y-6">
            <div><h2 className="font-display text-2xl font-bold text-foreground mb-2">Cosa succede quando studi?</h2><p className="text-muted-foreground">Non c'è niente di sbagliato.</p></div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {struggles.map((s) => (
                <button key={s.id} onClick={() => setData({ ...data, struggles: toggleInArray(data.struggles, s.id) })} className={`text-left px-4 py-4 rounded-2xl border transition-all ${data.struggles.includes(s.id) ? "border-primary bg-sage-light shadow-soft" : "border-border bg-card hover:bg-muted"}`}>
      ...
      case 4:
        return (
          <div className="space-y-6">
            <div><h2 className="font-display text-2xl font-bold text-foreground mb-2">Come vuoi che ti aiuti il coach?</h2></div>
            <div className="space-y-3">
              {supportStyles.map((s) => (
                <button key={s.id} onClick={() => setData({ ...data, supportStyle: s.id })} className={`w-full text-left px-5 py-4 rounded-2xl border transition-all ${data.supportStyle === s.id ? "border-primary bg-sage-light shadow-soft" : "border-border bg-card hover:bg-muted"}`}>
      ...
      case 5:
        return (
          <div className="space-y-6">
            <div><h2 className="font-display text-2xl font-bold text-foreground mb-2">Quanto riesci a concentrarti?</h2><p className="text-muted-foreground">Va benissimo anche poco.</p></div>
            <div className="space-y-3">
              {[
                ...
                <button key={opt.val} onClick={() => setData({ ...data, focusTime: opt.val })} className={`w-full text-left px-5 py-4 rounded-2xl border transition-all ${data.focusTime === opt.val ? "border-primary bg-sage-light shadow-soft" : "border-border bg-card hover:bg-muted"}`}>
                  <div className="flex items-center justify-between">
                    <div><span className="font-medium text-foreground">{opt.label}</span><span className="text-sm text-muted-foreground ml-2">— {opt.desc}</span></div>
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      <div className="px-6 pt-6 pb-4">
        ...
        <div className="max-w-lg mx-auto mt-4">
          <div className="h-1.5 bg-muted rounded-full overflow-hidden">
            <motion.div className="h-full bg-primary rounded-full" initial={{ width: 0 }} animate={{ width: `${((step + 1) / totalSteps) * 100}%` }} transition={spring} />
          </div>
        </div>
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-background/80 backdrop-blur-md border-t border-border px-6 py-4">
        <div className="max-w-lg mx-auto flex items-center justify-between">
          <Button variant="ghost" onClick={() => step > 0 ? setStep(step - 1) : navigate("/")} className="text-muted-foreground">
          ...
          <Button onClick={next} disabled={!canProceed() || saving} className="bg-primary text-primary-foreground hover:bg-sage-dark rounded-2xl px-6 disabled:opacity-40">
            {step === totalSteps - 1 ? (saving ? "Salvataggio..." : "Iniziamo!") : "Avanti"}
