...
export default function UnifiedSession() {
  ...
  // ── Activate deterministic math engine ──
  // Triggers when: user says "pronto"/ok/sì OR AI says "iniziamo con" OR topic has division + AI replied
  useEffect(() => {
    ...
    // Path 1: user said "pronto" or similar
    const lastUser = [...messages].reverse().find(m => m.role === "user");
    const userReady = lastUser && /pronto|sono pronto|iniziamo|proviamo|ok|sì|si|vai/i.test(lastUser.content.toLowerCase());
    
    // Path 2: AI said "iniziamo con" or similar (and at least 2 messages exist)
...
→ Aggiungi il tag [LINK_PREP:subject=${subject || ""};topic=<ARGOMENTO_SPECIFICO>;type=orale] nel messaggio. ⚠️ <ARGOMENTO_SPECIFICO> deve essere l'argomento CONCRETO trattato nella conversazione (es. "verbo sentire", "Teorema di Pitagora"), NON la categoria generica "${topic || subject}". Se l'esercizio era "ripetere il verbo X" → topic="verbo X". Mai usare "Argomento specifico" o solo il nome della materia.
