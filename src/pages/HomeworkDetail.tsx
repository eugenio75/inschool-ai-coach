...
import { ArrowLeft, ArrowRight, Check, BookOpen, Brain, Lightbulb, Play } from "lucide-react";
import { Button } from "@/components/ui/button";
import { mockTasks, subjectColors } from "@/lib/mockData";
...
const HomeworkDetail = () => {
  ...
  const [completedSteps, setCompletedSteps] = useState<string[]>(
    task.microSteps?.filter(s => s.done).map(s => s.id) || []
  );
  ...
  const progress = task.microSteps
    ? completedSteps.length / task.microSteps.length
    : 0;
  ...
  return (
    <div className="min-h-screen bg-background pb-32">
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="mt-5">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-muted-foreground">{completedSteps.length}/{task.microSteps?.length || 0} passi completati</span>
              ...
              return (
                ...
                >
                  <div className={`rounded-2xl border transition-all ${
                    done ? "bg-sage-light/30 border-primary/20" : "bg-card border-border shadow-soft"
                  }`}>
                    <button
                      onClick={() => toggleStep(step.id)}
                      className="w-full flex items-start gap-3 p-4 text-left"
                    >
                      <div className={`w-6 h-6 rounded-lg flex-shrink-0 flex items-center justify-center mt-0.5 transition-colors ${
                        done ? "bg-primary" : "border-2 border-border"
                      }`}>
                    ...
                    {step.hint && !done && (
                      <div className="px-4 pb-3">
                        <button
                          ...
                          className="flex items-center gap-1.5 text-xs text-clay-dark hover:text-foreground transition-colors"
                        >
                        ...
                        {showHint === step.id && (
                          <motion.p
                            ...
                            className="text-xs text-clay-dark bg-clay-light/50 rounded-xl px-3 py-2 mt-2"
                          >
                            💡 {step.hint}
                          </motion.p>
                        )}
      ...
      {/* Key concepts */}
      {task.keyConcepts && task.keyConcepts.length > 0 && (
        <div className="px-6 mt-8">
          <div className="max-w-2xl mx-auto">
            ...
            <div className="space-y-2">
              {task.keyConcepts.map((concept, i) => (
                <motion.div
                  key={i}
                  initial={{ opacity: 0, x: -10 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ ...spring, delay: 0.3 + i * 0.06 }}
                  className="bg-card rounded-xl border border-border px-4 py-3 shadow-soft"
                >
      ...
      {/* Recall questions */}
      {task.recallQuestions && task.recallQuestions.length > 0 && (
        <div className="px-6 mt-8">
          <div className="max-w-2xl mx-auto">
            <div className="flex items-center gap-2 mb-4">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-background/80 backdrop-blur-md border-t border-border px-6 py-4">
        <div className="max-w-2xl mx-auto flex gap-3">
          <Button
            ...
            className="flex-1 bg-primary text-primary-foreground hover:bg-sage-dark rounded-2xl py-5 text-base"
          >
            <Play className="w-4 h-4 mr-2" />
            Inizia sessione di focus
          </Button>
        </div>
      </div>
    </div>
